...
import { ShareholderType, type Address, type OrderData, type StepComponentProps } from '@/lib/types';
import { ChevronLeft, ChevronRight, Info, Loader2, PlusCircle, X } from 'lucide-react';
import type { FC } from 'react';

const Step2ProvideDetails: FC<StepComponentProps> = ({ orderData, updateOrderData, goToNextStep, goToPrevStep, isLoading, setIsLoading }) => {
  ...
  const addDirector = () => {
    ...
    toast({ title: "Director Added", description: "Great, you've added a new director. Please provide their details." });
  };
  const removeDirector = (index: number) => {
    updateOrderData(prev => ({
      directors: prev.directors?.filter((_, i) => i !== index)
    }));
    toast({ title: "Director Removed", description: `Director ${index + 1} has been successfully removed.`, variant: "default" });
  };
  ...
  const addShareholder = () => {
    ...
    toast({ title: "Shareholder Added", description: "Excellent, a new shareholder is on board. Let's fill in their info." });
  };
  const removeShareholder = (index: number) => {
    updateOrderData(prev => ({
      shareholders: prev.shareholders?.filter((_, i) => i !== index)
    }));
    toast({ title: "Shareholder Removed", description: `Shareholder ${index + 1} has been successfully removed.`, variant: "default" });
  };
  ...
  const handleShareholderTypeChange = (index: number, type: ShareholderType) => {
    ...
    toast({ title: "Shareholder Updated", description: `Shareholder ${index + 1} is now set as a ${type}.` });
  };

  const handleExtraRequestsChange = (value: string) => {
    updateOrderData({ extraRequests: value });
  };

  const handleProceed = () => {
    if (!orderData.companyNames?.firstChoice) {
      toast({
        title: "Company Name Needed!",
        description: "Every great company needs a name! Please provide at least your first choice for the company name to proceed.",
        variant: "destructive",
      });
      return;
    }
    ...
    toast({
      ...
      description: "All company details look good. Let's move on to review your order and complete the payment.",
    });
    goToNextStep();
  };
    ...
    <div className="space-y-8">
            ...
            {orderData.directors?.map((director, index) => (
              <Card key={director.id} className="overflow-hidden">
                <CardHeader className="flex flex-row items-center justify-between bg-muted/30 py-3 px-4">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => removeDirector(index)} className="h-7 w-7"><X className="h-4 w-4" /></Button>
                </CardHeader>
        ...
        <AccordionItem value="item-6">
          ...
          <AccordionContent className="pt-4">
            <Textarea
              ...
              onChange={e => handleExtraRequestsChange(e.target.value)}
      ...
      <div className="flex justify-between items-center mt-8 pt-6 border-t">
        ...
        <Button onClick={handleProceed} disabled={isLoading}>
          {isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "Review & Pay"} <ChevronRight className="ml-2 h-4 w-4" />
        </Button>
      </div>
    </div>
  );
