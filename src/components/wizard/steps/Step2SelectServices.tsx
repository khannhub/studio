...
import type { IncorporationDetails, IncorporationRecommendationItem, OrderData, StepComponentProps } from '@/lib/types';
...
import { ChevronLeft, ChevronRight, Loader2, Package, Settings, ShoppingBag } from 'lucide-react';
...
const Step2SelectServices: FC<StepComponentProps> = ({
  ...
  const handleIncorporationPackageSelect = (packageName: string, packagePrice: number) => {
    ...
    toast({
      title: "Package Added!",
      description: `The ${packageName} package is now part of your setup. Great benefits ahead!`,
    });
  };

  const handleAddonToggle = (addonId: string, selected: boolean, addonName: string) => {
    updateOrderData(prev => ({
    ...
    toast({
      title: `${selected ? 'Add-on Activated!' : 'Add-on Removed'}`,
      description: `${addonName} ${selected ? 'is now enhancing your order!' : 'has been taken off your list.'}`,
    });
  };
  ...
  const handleAddonInputChange = (addonId: string, field: keyof OrderData['addOns'][number], value: string) => {
    ...
    // Potentially add a debounced toast here if needed, e.g., "Banking preferences updated"
  };
  ...
  const handleProceedToDetails = () => {
    if (!orderData.incorporation?.jurisdiction || !orderData.incorporation.companyType || orderData.incorporation.price === undefined) {
      toast({
        title: "Just a Moment!",
        description: "Please make sure you've chosen your jurisdiction, company type, and a service package to move forward.",
        variant: "destructive"
      });
      return;
    }
      ...
      try {
        const prefillInput: PrefillCompanyDetailsInput = {
          ...
          companyNamePreferences: [
            orderData.companyNames?.firstChoice || '',
            orderData.companyNames?.secondChoice || '',
            orderData.companyNames?.thirdChoice || '',
          ].filter(name => name !== ''),
        };
        ...
        updateOrderData(prev => ({
          ...prev,
          companyNames: prefilledData.companyNames ? {
            firstChoice: prefilledData.companyNames[0] || prev.companyNames?.firstChoice,
            secondChoice: prefilledData.companyNames[1] || prev.companyNames?.secondChoice,
            thirdChoice: prefilledData.companyNames[2] || prev.companyNames?.thirdChoice,
          } : prev.companyNames,
          // Potentially prefill directors, shareholders if AI provides them
          // For now, focusing on company names as per AI flow output
        }));

        toast({
          title: "Smart Start!",
          description: "We've fast-tracked your setup by pre-filling some company details. Let's review and finalize them."
        });
        goToNextStep();
      } catch (error) {
        ...
        toast({
          title: "Manual Entry",
          description: "No worries! We couldn't prefill all details this time. Please proceed to enter them manually in the next step.",
          variant: "warning" // Changed to warning as it's not a critical failure
        });
        goToNextStep();
      } finally {
          ...
          <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {displayedPackages.map(pkg => (
              <Label
                ...
                onClick={() => handleIncorporationPackageSelect(pkg.name, pkg.price)}
                className={cn(
              ...
              <AccordionContent className="p-4 pt-0 space-y-3">
                ...
                <div className="flex items-center space-x-2">
                  <Switch
                    ...
                    onCheckedChange={(checked) => handleAddonToggle(addon.id, !!checked, addon.name)}
                  />
